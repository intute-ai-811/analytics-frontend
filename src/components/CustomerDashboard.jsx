...
const StatusPill = React.memo(({ status }) => {
  ...
  return (
    <span className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-md border ${styles[status] || styles.offline} flex items-center gap-2 w-fit`}>
      <span className={`w-1.5 h-1.5 rounded-full ${styles[status]?.includes("emerald") ? "bg-emerald-400" : "bg-gray-400"} animate-pulse`} />
      {label}
    </span>
  );
});
...
export default function CustomerDashboard() {
  ...
  const processedData = useMemo(() => {
    ...
    if (query.trim()) {
      const q = query.toLowerCase();
      data = data.filter((r) => r.vehicleType.toLowerCase().includes(q) || r.vehicleNo.toLowerCase().includes(q));
    }
    return data;
  }, [rows, query]);

  const totalPages = Math.max(1, Math.ceil(processedData.length / PAGE_SIZE));
  const paginatedRows = useMemo(() => processedData.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE), [processedData, page]);

  return (
    <div className="min-h-screen bg-[#0a0814] text-white p-4 md:p-8">
      <style>{`
        .glass { background: rgba(15, 12, 33, 0.7); backdrop-filter: blur(10px); border: 1px solid rgba(139, 92, 246, 0.2); }
        .cyber-text { background: linear-gradient(to right, #8b5cf6, #ec4899); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
      `}</style>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 flex items-center justify-between">
            <div>
                <h1 className="text-4xl font-black uppercase tracking-tighter cyber-text">Fleet Nexus</h1>
                <p className="text-gray-500 font-bold tracking-widest text-[10px] uppercase">Live Terminal Interface v2.4</p>
            </div>
            <button onClick={fetchData} className="p-4 rounded-2xl glass hover:border-purple-500 transition-all">
        ...
        <div className="relative mb-8">
            ...
            <input 
                ...
                placeholder="Query vehicle database..."
                className="w-full bg-[#151228] border border-white/10 rounded-2xl py-4 pl-12 pr-4 outline-none focus:border-purple-500 transition-all"
            />
        </div>
            ...
            <table className="w-full text-left">
                <thead className="bg-white/5">
                    <tr className="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-black">
                        ...
                        <th className="px-6 py-6">Reg No</th>
                        <th className="px-6 py-6">kWh/Hr</th>
