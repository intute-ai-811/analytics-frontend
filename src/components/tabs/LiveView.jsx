...
export default function LiveView() {
  ...
  const HoursToHrMin = ({ hours }) => {
    if (hours == null || hours === 0) return <span className="text-orange-300 font-semibold">0 hr 0 mins</span>;
  ...
  const Val = ({ v, unit = "", fixed = 2 }) => (
    <span className="text-orange-300 font-semibold">{v == null ? "–" : `${Number(v).toFixed(fixed)}${unit}`}</span>
  );
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold ${
            isActivelyLive ? "bg-emerald-600/30 text-emerald-300 border border-emerald-500/80" : "bg-gray-700/40 text-gray-400 border border-gray-600/60"
          }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Section title="Battery / BMS / Charger">
          ...
          <Item name="Temperature Sensors" value={
            <button onClick={() => setShowTempModal(true)} className="text-orange-400 font-semibold underline hover:text-orange-300 transition">
              View All → Modules
            </button>
          } />
          <Item name="Cell Voltages" value={
            <button onClick={() => setShowVoltageModal(true)} className="text-emerald-400 font-semibold underline hover:text-emerald-300 transition">
              View All → Modules
            </button>
          } />
          ...
          <div className="bg-gradient-to-br from-gray-900 via-gray-900 to-orange-950/30 rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden border border-orange-500/40 shadow-2xl shadow-orange-500/20" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 bg-gradient-to-b from-gray-800/60 to-gray-800/30 border-b border-orange-500/20">
              <div className="flex flex-wrap gap-3 justify-center mb-3">
                {[["Max", tempPackStats.max_c], ["Avg", tempPackStats.avg_c], ["Min", tempPackStats.min_c]].map(([label, val]) => (
                  <div key={label} className="px-4 py-2 rounded-lg border-2 bg-gray-800/80 shadow-inner" style={{ borderColor: tempColor(val) }}>
            ...
            <div className="overflow-y-auto max-h-[58vh] p-4">
              {(live.temp_modules ?? []).map((moduleValues, i) => {
                ...
                const moduleBorder = moduleStats.status === "CRITICAL" ? "border-red-500" : moduleStats.status === "WARN" ? "border-yellow-500/70" : "border-green-500/60";
                return (
                  <div key={i} className={`mb-5 bg-gray-800/40 rounded-lg p-4 border-2 ${moduleBorder}`}>
                    <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                      <h3 className="text-base font-bold text-orange-300">Module {i + 1}</h3>
                      <div className="flex gap-2 text-xs flex-wrap">
                        <span className="px-2.5 py-1 rounded bg-green-500/10 text-green-400 border border-green-500/30">Min: {fmt1(moduleStats.min_c)}°C</span>
                        <span className="px-2.5 py-1 rounded bg-red-500/10 text-red-400 border border-red-500/30">Max: {fmt1(moduleStats.max_c)}°C</span>
                        {moduleStats.status && (
                    ...
                    <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-1.5">
                      ...
                      {moduleValues.length === 0 && <div className="col-span-full text-center text-gray-500 py-8">No temperature data for this module</div>}
                    </div>
                  </div>
                );
              })}
        ...
        <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowVoltageModal(false)}>
          <div className="bg-gradient-to-br from-gray-900 via-gray-900 to-emerald-950/30 rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden border border-emerald-500/40 shadow-2xl shadow-emerald-500/20" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 bg-gradient-to-b from-gray-800/60 to-gray-800/30 border-b border-emerald-500/20">
              <div className="flex flex-wrap gap-3 justify-center mb-3">
                <div className="px-4 py-2 rounded-lg border-2 bg-gray-800/80 shadow-inner border-red-500"><div className="text-xs text-gray-400">Max</div><div className="text-lg font-bold text-red-400">{fmt3(voltagePackStats.max_v)}V</div></div>
                <div className="px-4 py-2 rounded-lg border-2 bg-gray-800/80 shadow-inner border-yellow-400"><div className="text-xs text-gray-400">Avg</div><div className="text-lg font-bold text-yellow-400">{fmt3(voltagePackStats.avg_v)}V</div></div>
                <div className="px-4 py-2 rounded-lg border-2 bg-gray-800/80 shadow-inner border-green-500"><div className="text-xs text-gray-400">Min</div><div className="text-lg font-bold text-green-400">{fmt3(voltagePackStats.min_v)}V</div></div>
                <div className="px-4 py-2 rounded-lg border-2 bg-gray-800/80 shadow-inner border-red-500"><div className="text-xs text-gray-400">Outliers</div><div className="text-lg font-bold text-red-400">±0.10V: {voltagePackStats.outliers ?? 0}</div></div>
              </div>
              <div className="flex flex-wrap gap-2 justify-center text-xs">
                  ...
                  <div key={i} className="mb-5 bg-gray-800/40 rounded-lg p-4 border border-emerald-500/20">
                    <div className="flex justify-between items-center mb-3 flex-wrap gap-2">
                      ...
                      <div className="flex gap-2 text-xs">
                        <span className="px-2.5 py-1 rounded bg-green-500/10 text-green-400 border border-green-500/30">Min: {fmt3(moduleStats.min_v)}V</span>
                        <span className="px-2.5 py-1 rounded bg-red-500/10 text-red-400 border border-red-500/30">Max: {fmt3(moduleStats.max_v)}V</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-1.5">
                      ...
                      {moduleValues.length === 0 && <div className="col-span-full text-center text-gray-500 py-8">No voltage data for this module</div>}
                    </div>
                  </div>
                );
              })}
