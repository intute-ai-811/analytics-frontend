      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Section title="Battery / BTMS / Charger Live Data">
          <Item
            ...
            value={<Val v={live.soc_percent} fixed={1} />}
          />
          <Item
            name="State of Charge (kWh)"
            value={<Val v={live.battery_kwh} unit=" kWh" />}
          />
          ...
          <Item
            ...
            value={live.charger_connected ?? "–"}
          />
          <Item
            name="Temperature Sensors (Module 1)"
            value={<Val v={live.max_cell_temp_c} unit="°C" />}
          />
          <Item
            name="Temperature Sensors (Module 2)"
            value={<Val v={live.avg_cell_temp_c} unit="°C" />}
          />
          <Item
            name="Temperature Sensors (Module 3)"
            value={<Val v={live.max_cell_temp_c} unit="°C" />}
          />
          <Item
            name="Temperature Sensors (Module 4)"
            value={<Val v={live.avg_cell_temp_c} unit="°C" />}
          />
          <Item
            name="Temperature Sensors (Module 5)"
            value={<Val v={live.max_cell_temp_c} unit="°C" />}
          />
          <Item
            name="Cell Voltages (Module 1)"
            value={<Val v={live.stack_voltage_v} unit=" V" fixed={2} />}
          />
          <Item
            name="Cell Voltages (Module 2)"
            value={<Val v={live.stack_voltage_v} unit=" V" fixed={2} />}
          />
          <Item
            name="Cell Voltages (Module 3)"
            value={<Val v={live.stack_voltage_v} unit=" V" fixed={2} />}
          />
          <Item
            name="Cell Voltages (Module 4)"
            value={<Val v={live.stack_voltage_v} unit=" V" fixed={2} />}
          />
          <Item
            name="Cell Voltages (Module 5)"
            value={<Val v={live.stack_voltage_v} unit=" V" fixed={2} />}
          />
          <Item
            name="Output Current (DC)"
          ...
          />
          <Item
            name="BTMS Temperature"
            value={<Val v={live.hyd_oil_temp_c} unit="°C" />}
          />
          <Item
        ...
        <Section title="Motor & MCU Live Data">
          ...
          <Item
            ...
            value={<Val v={live.motor_speed_rpm} unit=" rpm" fixed={0} />}
          />
          <Item
            name="Motor AC Side Current"
        ...
        <Section title="Peripherals Live Data">
          ...
          />
          <Item
            name="Hydraulic Pump RPM"
            value={<Val v={live.hyd_pump_rpm} unit=" rpm" fixed={0} />}
          />
          <Item
            name="Hydraulic Pump Motor Temperature"
            value={<Val v={live.motor_temp_c} unit="°C" />}
          />
          <Item
            name="DC/DC Converter Output Current"
            value={<Val v={live.dc_dc_current_a} unit=" A" fixed={1} />}
          />
        ...
        <Section title="ODO / Trip Details">
          <Item
            ...
            value={<Val v={live.total_hours} unit=" h" fixed={0} />}
          />
          <Item
            name="Last Trip Hours"
            value={<Val v={live.last_trip_hrs} unit=" h" fixed={1} />}
          />
          <Item
            name="Total kWh Consumed"
            value={<Val v={live.total_kwh} unit=" kWh" />}
          />
          <Item
            name="kWh Used in Last Trip"
            value={<Val v={live.last_trip_kwh} unit=" kWh" />}
          />
        </Section>
...
function MotorAnalytics() {
  ...
  const trip = useMemo(() => {
    if (!data.length) {
      return { maxPower: 0, maxTorque: 0, maxTemp: 0, kwh: 0 };
    }

    // Take latest trip record (assumed last element)
    const latest = data[data.length - 1];

    const maxPower = latest.max_op_power_kw || 0;
    const maxTorque = latest.max_op_torque_nm || 0;
    const maxTemp = latest.max_motor_temp_c || 0;
    const kwh = latest.total_kwh_consumed || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Section title="ODO">
          ...
          />
          <Stat
            label="Total kWh Consumed (30 days)"
            value={odo.totalEnergy.toFixed(1)}
            unit="kWh"
          />
          <Stat
            label="Avg Power per Day"
            value={odo.avgPower.toFixed(1)}
            unit="kW"
          />
        </Section>

        <Section title="Latest Trip Details">
          <Stat
            label="Max O/P power delivered in latest trip"
            value={trip.maxPower}
            unit="kW"
          />
          <Stat
            label="Max O/P torque delivered in latest trip"
            value={trip.maxTorque}
            unit="Nm"
          />
          <Stat
            label="Max motor temperature in latest trip"
            value={trip.maxTemp}
            unit="°C"
            warn={80}
            danger={95}
          />
          <Stat
            label="kWh consumed in latest trip"
            value={trip.kwh.toFixed(1)}
            unit="kWh"
          />
        </Section>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <ChartCard title="Max Power (kW) — Last 30 Days">
          <BarChart
            data={data.map((d) => ({ x: d.day, y: d.max_op_power_kw }))}
          />
        </ChartCard>
        <ChartCard title="Max Torque (Nm) — Last 30 Days">
          <BarChart
            data={data.map((d) => ({ x: d.day, y: d.max_op_torque_nm }))}
          />
        </ChartCard>
        <ChartCard title="Max Motor Temp (°C) — Last 30 Days">
          <BarChart
            data={data.map((d) => ({ x: d.day, y: d.max_motor_temp_c }))}
...
function BatteryAnalytics() {
  ...
  const odo = useMemo(() => {
    ...
    const avgTemp = data.length
      ? data.reduce((s, d) => s + (d.avg_cell_temp_c || 0), 0) / data.length
      : 0;
    const maxCurrent = Math.max(...data.map((d) => d.max_dc_current_a || 0), 0);
    const avgCurrent = data.length
      ? data.reduce((s, d) => s + (d.avg_dc_current_a || 0), 0) / data.length
      : 0;
  ...
  const trip = useMemo(() => {
    ...
    const avgTemp = last7.length
      ? last7.reduce((s, d) => s + (d.avg_cell_temp_c || 0), 0) / last7.length
      : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Section title="ODO">
          ...
          <Stat
            ...
            value={odo.avgTemp.toFixed(1)}
            unit="°C"
            warn={42}
            danger={52}
          />
          <Stat
            label="Avg DC Current"
            value={odo.avgCurrent.toFixed(1)}
            unit="A"
          />
        </Section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <ChartCard title="Daily kWh Consumed">
          <BarChart
            data={data.map((d) => ({ x: d.day, y: d.total_kwh_consumed }))}
          />
        </ChartCard>
        <ChartCard title="Max Cell Temp (°C)">
          <BarChart
            data={data.map((d) => ({ x: d.day, y: d.max_cell_temp_c }))}
            band={{ warn: 45, danger: 55 }}
          />
        </ChartCard>
        <ChartCard title="Avg Cell Temp (°C)">
          <BarChart
            data={data.map((d) => ({ x: d.day, y: d.avg_cell_temp_c }))}
...
function MotorFaults() {
  ...
  const counts = useMemo(() => {
    const map = new Map();
    logs.forEach((l) =>
      map.set(l.description, (map.get(l.description) || 0) + 1)
    );
    return map;
  }, [logs]);
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...filtered.map((l) => [l.recorded_at, l.code, l.description, l.status]),
    ];
    const csv = rows
  ...
  if (loading)
    return (
      <div className="text-center py-12 text-orange-200">Loading faults…</div>
    );
  if (error)
    return <div className="text-center py-12 text-red-400">Error: {error}</div>;
...
function DatabaseLogs() {
  ...
  const rows = useMemo(() => {
    ...
    return dates.map((date) => {
      ...
      const mkMin = rnd(55, 70, 1),
        mkMax = mkMin + rnd(10, 28, 1);
      const mcuMin = rnd(40, 55, 1),
        mcuMax = mcuMin + rnd(8, 22, 1);
      const batMin = rnd(28, 36, 1),
        batMax = batMin + rnd(4, 14, 1);
      const oilMin = rnd(30, 42, 1),
        oilMax = oilMin + rnd(8, 18, 1);
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `logs_${start}_to_${end}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[160px]">
      ...
      <text
        ...
        y={height - 5}
        textAnchor="middle"
        className="text-[10px] fill-orange-200/60"
      >
        Date
      </text>
    </svg>
