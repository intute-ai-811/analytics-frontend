        ...
        <div className="rounded-2xl border border-purple-500/30 bg-gray-800/40 backdrop-blur-md overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-purple-500/10">
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-16 text-center text-purple-300/70">
                    {loading ? "Loading…" : "No customers found"}
                  </td>
                </tr>
              ) : (
                filtered.map((r) => (
                  <tr key={r.customer_id} className="hover:bg-purple-500/5">
                    <td className="px-6 py-4 text-center font-semibold text-purple-200">{r.company_name}</td>
                    <td className="px-6 py-4 text-center text-gray-300">{r.address || "-"}</td>
                    <td className="px-6 py-4 text-center text-gray-300">{r.contact_person || "-"}</td>
                    <td className="px-6 py-4 text-center text-gray-300">{r.phone || "-"}</td>
                    <td className="px-6 py-4 text-center text-purple-300">{r.email}</td>
                    <td className="px-6 py-4 text-center">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => startEdit(r)} className="p-2 rounded-lg bg-purple-500/20 hover:bg-purple-500/30">
                          <Pencil size={16} />
                        </button>
                        <button onClick={() => askDelete(r.customer_id)} className="p-2 rounded-lg bg-red-500/20 hover:bg-red-500/30">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
