...
//       `${r.make} ${r.model} ${r.capacity_kwh} ${r.motor_specs} ${r.architecture_diagram}`
//         .toLowerCase()
//         .includes(q)
//     );
//   }, [rows, query]);
...
//                     <td className="px-6 py-4 text-sm">{r.motor_specs || "-"}</td>
...
//                 <TextArea label="Motor Specs" value={editing.motor_specs} onChange={v => setEditing({...editing, motor_specs: v})} />
...
export default function VehicleTypeMaster() {
  ...
  const saveEdit = async () => {
    ...
    try {
      if (!editing.vtype_id) {
        ...
        setRows((prev) =>
          prev.map((r) =>
            r.vtype_id === editing.vtype_id ? { ...r, ...payload } : r
          )
        );
      }
      setEditing(null);
  ...
  const exportCsv = () => {
    const headers =
      "Make,Model,Capacity (kWh),Specs,Wheel Loader,Excavator,Architecture,Drawings URL\n";
    const csv = rows
      .map((r) =>
        [
          r.make,
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex gap-4">
                  <label>
                    <input
                      type="checkbox"
                      checked={editing.wheel_loader}
                      onChange={(e) =>
                        setEditing({
                          ...editing,
                          wheel_loader: e.target.checked,
                        })
                      }
                    />{" "}
                    Wheel Loader
                  ...
                  <label>
                    <input
                      type="checkbox"
                      checked={editing.excavator}
                      onChange={(e) =>
                        setEditing({ ...editing, excavator: e.target.checked })
                      }
                    />{" "}
                    Excavator
                  </label>
