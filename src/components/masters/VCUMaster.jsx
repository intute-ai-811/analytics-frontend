...
export default function VCUMaster() {
  ...
  const fetchData = async () => {
    ...
    try {
      const { data } = await axios.get(API_BASE, { headers });
      console.log("VCU DATA LOADED:", data);
      setRows(Array.isArray(data) ? data : []);
    } catch (e) {
      const msg = e.response?.data?.error || e.message;
      setError(msg.includes("Unauthorized") ? "Session expired!" : msg);
  ...
  // Filter VCU rows based on search query
  const filtered = useMemo(() => {
    if (!query.trim()) return rows;
    const q = query.toLowerCase();
    return rows.filter(r =>
      `${r.vcu_make} ${r.vcu_model}`.toLowerCase().includes(q)
    );
  }, [rows, query]);

  // Open the modal for creating a new VCU
  const openNew = () => {
    setEditing(null);
    setForm({ vcu_make: "", vcu_model: "", vcu_specs: "" });
    setShowModal(true);
  };

  // Open the modal for editing an existing VCU
  const openEdit = (r) => {
    setEditing(r);
    setForm({
      vcu_make: r.vcu_make || "",
  ...
  // Remove a row (VCU)
  const removeRow = async (id) => {
    if (!confirm("Delete this VCU configuration?")) return;
    try {
      await axios.delete(`${API_BASE}/${id}`, { headers: getAuthHeaders() });
      setRows(prev => prev.filter(r => r.vcu_hmi_id !== id));
    } catch (e) {
  ...
  // Export VCU data as CSV
  const exportCSV = () => {
    const headers = "VCU Make,VCU Model,VCU Specs\n";
    const csv = rows.map(r => [
      r.vcu_make, r.vcu_model, r.vcu_specs || ""
    ].map(v => `"${v}"`).join(",")).join("\n");
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-gray-900 via-gray-900 to-black overflow-x-hidden text-white">
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-orange-300">Configure vehicle control units</p>
        </div>
        ...
        {loading && <div className="text-center py-8">Loading VCU...</div>}
        {error && (
          <div className="p-4 bg-red-900/50 border border-red-500 rounded-xl flex items-center gap-3">
            <AlertCircle className="w-5 h-5" />
            <span>{error}</span>
        ...
        <div className="bg-gray-800/50 rounded-2xl border border-orange-500/30 overflow-hidden">
          <table className="w-full">
            <thead className="bg-black/50">
              <tr>
                {["VCU Make", "VCU Model", "VCU Specs", "Actions"].map(h => (
                  <th key={h} className="px-6 py-4 text-left text-orange-200 font-semibold">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-16 text-orange-400">No VCU configurations</td></tr>
              ) : (
                filtered.map(r => (
                  <tr key={r.vcu_hmi_id} className="border-t border-orange-500/10 hover:bg-orange-500/5">
                    <td className="px-6 py-4 font-bold">{r.vcu_make}</td>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
            <div className="bg-gray-900 p-8 rounded-2xl border-2 border-orange-500 w-full max-w-3xl">
              <h2 className="text-2xl font-bold text-orange-300 mb-6">
                {editing ? "Edit" : "Add"} VCU Configuration
              </h2>
              <div className="grid grid-cols-2 gap-4">
                <Input label="VCU Make *" value={form.vcu_make} onChange={v => setForm({...form, vcu_make: v})} />
                <Input label="VCU Model *" value={form.vcu_model} onChange={v => setForm({...form, vcu_model: v})} />
