...
//     if (!form.vcu_id && vcuUnassigned) { setSelectedVcu(null); setVcuSearch(""); setVcuUnassigned(false); return; }
...
//     if (!form.hmi_id && hmiUnassigned) { setSelectedHmi(null); setHmiSearch(""); setHmiUnassigned(false); return; }
...
//   useEffect(() => {
//     if (!form.vcu_id && vcuUnassigned) { setForm((prev) => ({ ...prev, vcu_make_model: "" })); return; }
//     if (editing && Number(form.vcu_id) === Number(editing.vcu_id)) { setForm((prev) => ({ ...prev, vcu_make_model: editing.vcu_make_model || "" })); return; }
//     if (!form.vcu_id) return;
//     const vcu = vcus.find((v) => v.vcu_id === Number(form.vcu_id));
//     if (!vcu) return;
//     setForm((prev) => ({ ...prev, vcu_make_model: `${vcu.vcu_make || ""} ${vcu.vcu_model || ""}`.trim() }));
//   }, [form.vcu_id, vcus, editing, vcuUnassigned]);

//   useEffect(() => {
//     if (!form.hmi_id && hmiUnassigned) { setForm((prev) => ({ ...prev, hmi_make_model: "" })); return; }
//     if (editing && Number(form.hmi_id) === Number(editing.hmi_id)) { setForm((prev) => ({ ...prev, hmi_make_model: editing.hmi_make_model || "" })); return; }
//     if (!form.hmi_id) return;
//     const hmi = hmis.find((h) => h.hmi_id === Number(form.hmi_id));
//     if (!hmi) return;
//     setForm((prev) => ({ ...prev, hmi_make_model: `${hmi.hmi_make || ""} ${hmi.hmi_model || ""}`.trim() }));
...
//   const openNew = () => {
//     setEditing(null);
//     setSelectedVcu(null); setSelectedHmi(null);
//     setVcuSearch(""); setHmiSearch("");
//     setVcuUnassigned(false); setHmiUnassigned(false);
//     const year = new Date().getFullYear().toString().slice(2);
//     const nextNum = String(rows.length + 1).padStart(4, "0");
//     setForm({
//       vehicle_unique_id: `ERDE-${year}-${nextNum}`,
//       vehicle_reg_no: "", customer_id: "", vtype_id: "",
//       vcu_id: "", hmi_id: "", vcu_make_model: "", hmi_make_model: "",
...
//       hyd_cooling_yesno: "No", compressor_yesno: "No", motor_cooling_yesno: "No",
//       motor_controller_details: "", compressor_details: "", motor_cooling_details: "",
//       date_of_deployment: new Date().toISOString().split("T")[0],
//     });
//     setShowModal(true);
//   };
...
//     setSelectedVcu(vcu || null); setSelectedHmi(hmi || null);
//     setVcuSearch(vcu?.serial_number || ""); setHmiSearch(hmi?.imei_number || "");
//     setVcuUnassigned(false); setHmiUnassigned(false);
//     setForm({
...
//       value: v.vcu_id, serial: v.serial_number,
...
//       value: h.hmi_id, imei: h.imei_number,
...
//       vcu_id: form.vcu_id !== "" ? Number(form.vcu_id) : editing ? editing.vcu_id : null,
//       hmi_id: form.hmi_id !== "" ? Number(form.hmi_id) : editing ? editing.hmi_id : null,
...
//       } else {
//         await axios.put(`${VEHICLE_API}/${editing.vehicle_master_id}`, payload, { headers: getAuthHeaders() });
//       }
//       await fetchVehicles();
//       setShowModal(false);
//     } catch (e) {
...
//   const exportCSV = () => {
//     const headers = ["Unique ID","Reg No.","Customer","Vehicle Type","VCU Serial","HMI IMEI","VCU Make+Model","HMI Make+Model","Motor","Battery","DC/DC","BTMS","Deployment","Actions"];
//     const csv = [
//       headers.join(","),
//       ...rows.map((r) =>
//         [
...
//           r.vcu_serial || "", r.hmi_imei || "", r.vcu_make_model || "", r.hmi_make_model || "",
//           r.motor_make_model || "", r.battery_make_model || "", r.dc_dc_make_model || "", r.btms_make_model || "",
//           r.hyd_cooling_yesno ? "Yes" : "No", r.compressor_yesno ? "Yes" : "No", r.motor_cooling_yesno ? "Yes" : "No",
...
//               <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search fleet..."
...
//               <tr>
//                 {["Unique ID","Reg No.","Customer","Vehicle Type","VCU Serial","HMI IMEI","VCU Make+Model","HMI Make+Model","Motor","Battery","DC/DC","BTMS","Deployment","Actions"].map((h) => (
...
//               ))}
//               {filtered.length === 0 && (
//                 <tr><td colSpan={14} className="text-center py-16 text-orange-400">No vehicles found</td></tr>
//               )}
//             </tbody>
//           </table>
//         </div>
...
//                   <Input label="Reg No./VIN *" value={form.vehicle_reg_no} onChange={(v) => setForm({ ...form, vehicle_reg_no: v })} />

//                   <Select label="Customer *" value={form.customer_id} onChange={(v) => setForm({ ...form, customer_id: v })}
//                     options={customers.map((c) => ({ value: c.customer_id, label: c.company_name }))} placeholder="Select customer" />

//                   <Select label="Vehicle Type *" value={form.vtype_id} onChange={(v) => setForm({ ...form, vtype_id: v })}
//                     options={vehicleTypes.map((vt) => ({ value: vt.vtype_id, label: `${vt.make} ${vt.model}` }))} placeholder="Select vehicle type" />

//                   {/* VCU Custom Dropdown */}
//                   <div className="relative">
//                     <label className="block"><span className="text-orange-300 text-xs">VCU Serial</span></label>
//                     <div className="relative">
//                       <input type="text" value={vcuSearch} onChange={(e) => setVcuSearch(e.target.value)}
...
//                         onFocus={() => setShowVcuList(true)} onBlur={() => setTimeout(() => setShowVcuList(false), 200)} />
//                       <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-orange-400 pointer-events-none" />
//                       {form.vcu_id && (
//                         <button type="button" onClick={unassignVcu}
//                           className="absolute right-10 top-1/2 -translate-y-1/2 text-gray-400 hover:text-orange-400 text-xl leading-none">×</button>
//                       )}
//                     </div>
//                     {selectedVcu && (
//                       <p className="text-xs text-orange-400 mt-1">
//                         Selected: {selectedVcu.serial_number}
//                         {selectedVcu.is_assigned && selectedVcu.assigned_vehicle_unique_id !== form.vehicle_unique_id &&
...
//                                 setSelectedVcu({ vcu_id: opt.value, serial_number: opt.serial, is_assigned: !!opt.assignedTo, assigned_vehicle_unique_id: opt.assignedTo });
//                                 setVcuSearch(opt.serial || "");
//                                 setShowVcuList(false);
//                               }}>
//                               <span>{opt.serial}{opt.isCurrent && <span className="text-xs text-green-400 ml-2">current</span>}</span>
//                               {opt.assignedTo && !opt.isCurrent && <span className="text-xs text-orange-400">→ {opt.assignedTo}</span>}
//                             </div>
//                           ))}
//                         {getVcuOptions().filter((opt) => (opt.serial || "").toLowerCase().includes(vcuSearch.toLowerCase())).length === 0 && (
//                           <div className="px-3 py-3 text-gray-400 text-sm text-center">No matching serial number</div>
//                         )}
//                       </div>
//                     )}
//                   </div>

//                   <Input label="VCU Make+Model (Snapshot)" value={form.vcu_make_model} disabled />

//                   {/* HMI Custom Dropdown */}
//                   <div className="relative">
//                     <label className="block"><span className="text-orange-300 text-xs">HMI IMEI</span></label>
//                     <div className="relative">
//                       <input type="text" value={hmiSearch} onChange={(e) => setHmiSearch(e.target.value)}
//                         placeholder="Type or select IMEI number..."
//                         className="mt-1 w-full px-3 py-2 pr-10 rounded-lg bg-gray-800 border border-orange-500/30 focus:ring-2 focus:ring-orange-500 text-sm"
//                         onFocus={() => setShowHmiList(true)} onBlur={() => setTimeout(() => setShowHmiList(false), 200)} />
//                       <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-orange-400 pointer-events-none" />
//                       {form.hmi_id && (
//                         <button type="button" onClick={unassignHmi}
//                           className="absolute right-10 top-1/2 -translate-y-1/2 text-gray-400 hover:text-orange-400 text-xl leading-none">×</button>
//                       )}
//                     </div>
//                     {selectedHmi && (
...
//                         {selectedHmi.is_assigned && selectedHmi.assigned_vehicle_unique_id !== form.vehicle_unique_id &&
...
//                                 setSelectedHmi({ hmi_id: opt.value, imei_number: opt.imei, is_assigned: !!opt.assignedTo, assigned_vehicle_unique_id: opt.assignedTo });
//                                 setHmiSearch(opt.imei || "");
//                                 setShowHmiList(false);
//                               }}>
//                               <span>{opt.imei}{opt.isCurrent && <span className="text-xs text-green-400 ml-2">current</span>}</span>
//                               {opt.assignedTo && !opt.isCurrent && <span className="text-xs text-orange-400">→ {opt.assignedTo}</span>}
//                             </div>
//                           ))}
//                         {getHmiOptions().filter((opt) => (opt.imei || "").toLowerCase().includes(hmiSearch.toLowerCase())).length === 0 && (
//                           <div className="px-3 py-3 text-gray-400 text-sm text-center">No matching IMEI number</div>
//                         )}
//                       </div>
//                     )}
//                   </div>

//                   <Input label="HMI Make+Model (Snapshot)" value={form.hmi_make_model} disabled />
...
//                   <YesNoSelect label="Hydraulic Cooling" value={form.hyd_cooling_yesno} onChange={(v) => setForm({ ...form, hyd_cooling_yesno: v })} />
//                   {form.hyd_cooling_yesno === "Yes" && (
//                     <TextArea label="Hydraulic Cooling Details" value={form.motor_controller_details} onChange={(v) => setForm({ ...form, motor_controller_details: v })} />
//                   )}

//                   <YesNoSelect label="Compressor" value={form.compressor_yesno} onChange={(v) => setForm({ ...form, compressor_yesno: v })} />
//                   {form.compressor_yesno === "Yes" && (
//                     <TextArea label="Compressor Details" value={form.compressor_details} onChange={(v) => setForm({ ...form, compressor_details: v })} />
//                   )}

//                   <YesNoSelect label="Motor Cooling" value={form.motor_cooling_yesno} onChange={(v) => setForm({ ...form, motor_cooling_yesno: v })} />
//                   {form.motor_cooling_yesno === "Yes" && (
//                     <TextArea label="Motor Cooling Details" value={form.motor_cooling_details} onChange={(v) => setForm({ ...form, motor_cooling_details: v })} />
//                   )}

//                   <Input label="Date of Deployment" type="date" value={form.date_of_deployment} onChange={(v) => setForm({ ...form, date_of_deployment: v })} />
//                 </div>
...
//           disabled ? "bg-gray-800/50 border-orange-500/20 text-gray-400 cursor-not-allowed" : "bg-gray-800 border-orange-500/30"
//         }`} {...props} />
//     </label>
//   );
// }
...
//       options={[{ value: "No", label: "No" }, { value: "Yes", label: "Yes" }]} placeholder="Select..." />
//   );
// }

// src/components/masters/VehicleMaster.jsx
import React, { useEffect, useMemo, useState } from "react";
import {
  Search,
  Download,
...
export default function VehicleMaster() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [veh, cust, vt, vcu, hmi] = await Promise.all([
        axios.get(VEHICLE_API, { headers: getAuthHeaders() }),
        axios.get(CUSTOMERS_API, { headers: getAuthHeaders() }),
        axios.get(VTYPES_API, { headers: getAuthHeaders() }),
        axios.get(VCU_API, { headers: getAuthHeaders() }),
        axios.get(HMI_API, { headers: getAuthHeaders() }),
  ...
  const saveForm = async () => {
    try {
      if (!editing) {
        await axios.post(VEHICLE_API, form, { headers: getAuthHeaders() });
      } else {
        await axios.put(`${VEHICLE_API}/${editing.vehicle_master_id}`, form, {
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 rounded-xl bg-black/40 border border-purple-500/30 focus:border-purple-500"
            />
          </div>
        ...
        <div className="rounded-2xl border border-purple-500/30 bg-gray-800/40 backdrop-blur-md overflow-hidden">
          <table className="w-full">
            <thead className="bg-black/50 text-purple-200">
              <tr>
                {["ID","Reg No","Customer","Type","Actions"].map((h) => (
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.vehicle_master_id} className="hover:bg-purple-500/5">
                  <td className="text-center">{r.vehicle_unique_id}</td>
                  <td className="text-center">{r.vehicle_reg_no}</td>
                  <td className="text-center">{r.company_name}</td>
                  <td className="text-center">{r.vehicle_make} {r.vehicle_model}</td>

                  <td className="flex justify-center gap-2 py-3">
                    <button onClick={() => openEdit(r)} className="p-2 bg-purple-500/20 rounded">
                      <Pencil size={16}/>
                    </button>
                    <button onClick={() => deleteRow(r.vehicle_master_id)} className="p-2 bg-red-500/20 rounded">
                      <Trash2 size={16}/>
                    </button>
                  </td>
                </tr>
        ...
        {showModal && (
          <Modal title={editing ? "Edit Vehicle" : "Add Vehicle"} onClose={() => setShowModal(false)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Input label="Vehicle ID" value={form.vehicle_unique_id} onChange={(v)=>setForm({...form,vehicle_unique_id:v})}/>
              ...
              <Select label="Vehicle Type" value={form.vtype_id} onChange={(v)=>setForm({...form,vtype_id:v})}
                options={vehicleTypes.map(v=>({value:v.vtype_id,label:`${v.make} ${v.model}`}))}/>
            </div>

            <div className="flex justify-end gap-3 mt-6">
              <button onClick={()=>setShowModal(false)} className="px-5 py-2 border border-purple-500/30 rounded-xl">Cancel</button>
              <button onClick={saveForm} className="px-6 py-2 bg-gradient-to-r from-purple-500 to-fuchsia-500 rounded-xl font-bold">Save</button>
            </div>
  ...
  return (
    <label>
      ...
      <input
        ...
        className="mt-2 w-full px-4 py-3 rounded-xl bg-black/40 border border-purple-500/30 focus:border-purple-500"
      />
    </label>
  );
}
...
function Select({ label, value, onChange, options=[] }) {
  return (
    <label>
      <span className="text-sm text-purple-300">{label}</span>
      <select value={value || ""} onChange={(e)=>onChange(e.target.value)}
        className="mt-2 w-full px-4 py-3 rounded-xl bg-black/40 border border-purple-500/30">
