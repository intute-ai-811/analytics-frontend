...
const C = {
  bgGradient: "bg-gradient-to-br from-[#0b0b18] via-[#111124] to-[#1a1035]",
...
const StatusPill = React.memo(({ status }) => {
  const styles = status === "online" ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-500/20 text-slate-400";
  return (
    <span className={`px-3 py-1 text-xs font-bold rounded-full ${styles}`}>
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    if (!token) return;
    if (analyticsMode === "custom" && (!fromDate || !toDate)) return;
  ...
  const sortedRows = useMemo(() => {
    const filtered = rows.filter((r) => Object.values(r).some((v) => String(v).toLowerCase().includes(searchInput.toLowerCase())));
  ...
  }, [rows, searchInput, sortConfig]);

  return (
    <div className={`min-h-screen ${C.bgGradient} ${C.textPrimary} px-6 py-10`}>
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col items-center justify-center space-y-2">
          <h1 className="text-5xl font-extrabold tracking-tighter uppercase text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-fuchsia-500 to-indigo-400 drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]">
            Fleet Overview
          </h1>
          <div className="h-1 w-24 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full" />
        </div>

        <div className="flex flex-wrap gap-4 items-center justify-between bg-black/30 p-4 rounded-2xl border border-purple-500/20">
          <div className="relative flex-1 min-w-[300px]">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-purple-400" />
            <input
              className="w-full pl-12 pr-4 py-3 rounded-xl bg-black/40 border border-purple-500/30 focus:border-purple-400 outline-none transition-all"
              placeholder="Search registry (Vehicle #, Customer, Type)..."
              onChange={(e) => setSearchInput(e.target.value)}
            />
          </div>

          <div className="flex items-center gap-2">
            <div className="flex bg-black/60 p-1 rounded-lg border border-purple-500/30">
              {["all", "today", "custom"].map((m) => (
                <button key={m} onClick={() => setAnalyticsMode(m)} className={`px-5 py-2 rounded text-xs font-bold uppercase transition ${analyticsMode === m ? "bg-purple-600" : ""}`}>
                  {m}
                </button>
              ))}
            </div>
            {analyticsMode === "custom" && (
              <div className="flex items-center justify-between gap-2 bg-black/60 px-4 py-2.5 rounded-lg border border-purple-500/30 w-[360px]">
                <input type="date" className="bg-transparent text-xs text-white outline-none cursor-pointer flex-1 text-center" onChange={(e) => setFromDate(e.target.value)} />
                <div className="flex justify-center items-center px-2">
                  <span className="text-purple-600 font-black text-[10px] italic">TO</span>
                </div>
                <input type="date" className="bg-transparent text-xs text-white outline-none cursor-pointer flex-1 text-center" onChange={(e) => setToDate(e.target.value)} />
              </div>
            )}
          </div>
        </div>

        <div className={`rounded-2xl border ${C.border} ${C.cardBg} overflow-hidden`}>
          <table className="w-full text-sm">
            <thead className="bg-black/50 text-purple-200">
              <tr>{columns.map((c) => <th key={c.key} className="px-6 py-4 cursor-pointer" onClick={() => c.sortable && setSortConfig({ key: c.key, direction: sortConfig.direction === "asc" ? "desc" : "asc" })}>{c.label}</th>)}</tr>
            </thead>
            <tbody>
              {loading ? <tr><td colSpan={10} className="text-center py-10">Loading...</td></tr> : 
               sortedRows.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((r, i) => (
                <tr key={r.vehicle_master_id} onClick={() => handleRowClick(r)} className="hover:bg-purple-500/5 cursor-pointer transition">
                  <td className="px-6 py-4 text-center">{(page - 1) * PAGE_SIZE + i + 1}</td>
                  <td className="px-6 py-4 text-center">{r.vehicle_type}</td>
                  <td className="px-6 py-4 text-center">{r.capacity}</td>
                  <td className="px-6 py-4 text-center font-bold text-purple-300">{r.vehicle_no}</td>
                  <td className="px-6 py-4 text-center">{r.customer}</td>
                  <td className="px-6 py-4 text-center">{r.total_hours}</td>
                  <td className="px-6 py-4 text-center">{r.total_kwh}</td>
                  <td className="px-6 py-4 text-center">{r.avg_kwh}</td>
                  <td className="px-6 py-4 text-center"><StatusPill status={r.status} /></td>
                  <td className="px-6 py-4 text-center">
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/vehicle/${r.vehicle_master_id}/track`); }} className="px-3 py-1 bg-purple-500/20 rounded hover:bg-purple-500/40">
                      <MapPin size={14} />
                    </button>
                  </td>
                </tr>
